            ...
            plants.map((plant) => {
              ...
              return (
                <Card key={plant.id} className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg border-slate-200 bg-white hover:-translate-y-1 flex flex-col cursor-pointer ring-0 hover:ring-2 hover:ring-emerald-300" onClick={() => setSelectedPlantId(plant.id)}>
                  ...
                  <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-5 grow flex flex-col">
                    <div><CardTitle className="text-sm sm:text-base md:text-xl font-bold text-slate-900 leading-tight truncate">{plant.name}</CardTitle></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 sm:gap-y-3 gap-x-2 text-[10px] sm:text-xs pt-1">
                      <div className="flex items-center text-slate-600"><Droplets className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-sky-500" /><span className="font-medium truncate">{daysAgo === 0 ? t("watered_today") : `${daysAgo} ${t("watered_days_ago")}`}</span></div>
                      {plant.nextWateringDate && (
                        <div className="flex items-center text-slate-600 col-span-1 sm:col-span-2 bg-sky-50/50 p-1.5 rounded-md border border-sky-100/50 mt-0.5">
                          <Droplets className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-sky-400" />
                          <span className="font-medium text-[9px] sm:text-[11px] truncate">
                      ...
                      <div className="flex items-center text-slate-600"><Activity className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-emerald-500" /><span className={`font-medium truncate ${plant.health === "Excellent" ? "text-emerald-600" : plant.health === "Good" ? "text-emerald-500" : "text-amber-500"}`}>{plant.health === "Excellent" ? t("health_excellent") : plant.health === "Good" ? t("health_good") : t("health_needs_attention")}</span></div>
                      <div className="flex items-center text-slate-600"><MapPin className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-indigo-400" /><span className="font-medium truncate">{plant.locationType === "Indoor" ? t("location_indoor") : t("location_outdoor")}</span></div>
                      <div className="flex items-center text-slate-600"><Sun className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-amber-400" /><span className="font-medium truncate">{plant.lightExposure === "Low Light" ? t("light_low") : plant.lightExposure === "Partial Shade" ? t("light_partial") : plant.lightExposure === "Bright Indirect" ? t("light_bright") : t("light_full")}</span></div>
                      <div className="flex items-center text-slate-600"><Box className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-amber-700/60" /><span className="font-medium truncate">{plant.potType === "Terracotta" ? t("pot_terracotta") : plant.potType === "Plastic" ? t("pot_plastic") : plant.potType === "Ceramic" ? t("pot_ceramic") : plant.potType === "Metal" ? t("pot_metal") : t("pot_other")}</span></div>
                      <div className="flex items-center text-slate-600"><Droplets className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5 sm:me-2 text-slate-300" /><span className="font-medium truncate flex items-center gap-1">{t("has_drainage")}{plant.hasDrainage ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> : <X className="h-3.5 w-3.5 text-red-400" />}</span></div>
                    </div>
                    {plant.recentlyReplanted && <div className="pt-2 border-t border-slate-100 flex items-center text-amber-600 text-[10px] sm:text-xs font-medium"><Sprout className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 me-1.5" />{t("recently_replanted")}</div>}
                  </CardContent>
                </Card>
              )
            })
