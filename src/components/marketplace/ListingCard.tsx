...
export function ListingCard({ listing, ownerName, onClick }: ListingCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group text-start relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col ${
    ...
    >
      <div className="relative w-full aspect-[4/5] bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-3 start-3 flex flex-wrap gap-1.5">
          <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-white/95 backdrop-blur-sm text-slate-700 px-2 py-1 rounded-md shadow-sm">
