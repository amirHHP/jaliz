...
export function ListingCard({ listing, ownerName, onClick }: ListingCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group text-start relative overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col ${
        isCompleted ? "opacity-75" : ""
      }`}
    >
      <div className="relative w-full aspect-[4/5] bg-slate-100 dark:bg-slate-700 overflow-hidden">
        {listing.image ? (
        ...
        <div className="absolute top-3 start-3 flex flex-wrap gap-1.5">
          <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-card/95 backdrop-blur-sm text-card-foreground px-2 py-1 rounded-md shadow-sm">
            <TypeIcon className="h-3 w-3" />
            {t(TYPE_TRANSLATION_KEY[listing.type] as never)}
          </span>
          <span
            className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md ${MODE_BADGE_CLASS[listing.mode]}`}
          >
      ...
      <div className="p-3 sm:p-4 grow flex flex-col">
        <h3 className="font-semibold text-xs sm:text-sm md:text-base text-card-foreground line-clamp-1">
          {listing.title}
        </h3>
        <p className="mt-1 text-[11px] sm:text-sm text-muted line-clamp-2 leading-relaxed">
          {listing.description}
        </p>
